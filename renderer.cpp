...
void Renderer::Init()
{
	...
	bufferDesc.ByteWidth = sizeof(ATMOSPHERE);

	m_Device->CreateBuffer(&bufferDesc, NULL, &m_AtmosphereBuffer);
	...
	// カメラ初期化
	CAMERA camera{};
	camera.Position = D3DXVECTOR4(0.0f, 2.0f, -5.0f, 1.0f);
	SetCamera(camera);

	ATMOSPHERE atmos{};
	atmos._InnerRadius = 10000;
	atmos._OuterRadius = 10250;
	atmos._Kr = 0.0025;
	atmos._Km = 0.001;
	atmos.fSamples = 8.0;
...
void Renderer::Begin()
{
	float clearColor[4] = { 0.2f, 1.0f, 0.5f, 1.0f };
